...
import { SUBMIT_MESSAGE_UPDATES } from "../../Api/ApiConstants";
...
export const Messages = (messagesProps: MessagesProp) => {
  ...
  const sendMessage = (e: React.FormEvent<HTMLFormElement> | any) => {
    ...
    const request = { type: "MESSAGE", message };
    e.target.reset();
  ...
  useEffect(() => {
    ...
    setTimeout(() => messageElements[messageElements.length - 1].scrollIntoView(), 500);
  }, [messages]);
